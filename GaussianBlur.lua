...
function kernel(radius, sigma)
	...
	-- used for normalization of all weights (see below [*])
	...
	-- if radius is even then last weight is left without a pair (see below [**])
...
local cur = {
	Texture = dxCreateScreenSource(ScreenSizeX,ScreenSizeX),
	SizeX = ScreenSizeX,
	SizeY = ScreenSizeY
} -- DecodeANI("Testers/aliendance.ani")[1][5];

local s,t = dxCreateShader("GaussianBlur.fx");

dxSetShaderValue(s,"TextureSize",cur.SizeX,cur.SizeY);
...
local function render()
	...
	dxSetShaderValue(s,"Texture",cur.Texture);
...
addCommandHandler("blur",function(Command,Radius,Sigma)
	...
	local Limit,Weights,Offsets = kernel(Radius,Sigma);
